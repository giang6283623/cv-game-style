...
const SkillsPage: React.FC = () => {
  ...
  const technicalSkillLevels: { [key: string]: { level: number; icon?: React.ReactNode } } = {
    'React': { level: 95, icon: <FaReact /> },
    'NextJS': { level: 90, icon: <SiNextdotjs /> },
  ...
  return (
    <div className="skills-page">
      ...
      >
        <div style={{ 
          ...
          <div style={{ textAlign: 'center' }}>
            <h2 style={{ color: '#ffd93d', marginBottom: '10px' }}>DEVELOPER CLASS</h2>
            <p style={{ 
            ...
            return (
              ...
              >
                <SkillBar
                  skill={skill}
                  level={skillData.level}
                  color={index % 3 === 0 ? 'warning' : 
                         index % 3 === 1 ? 'success' : 'primary'}
                  icon={skillData.icon}
                  animated={true}
                />
              </motion.div>
      ...
      >
        <h3 style={{ color: '#ffd93d', marginBottom: '20px' }}>SKILL LEVEL LEGEND</h3>
        <div style={{ 
          display: 'flex', 
          justifyContent: 'center', 
          gap: '30px',
          ...
          <span><span style={{ color: '#ff6b6b' }}>0-20%</span> NOVICE</span>
          <span><span style={{ color: '#ffb800' }}>20-40%</span> BEGINNER</span>
          <span><span style={{ color: '#ffd93d' }}>40-60%</span> INTERMEDIATE</span>
          <span><span style={{ color: '#4ecdc4' }}>60-75%</span> ADVANCED</span>
          <span><span style={{ color: '#6bcf7f' }}>75-90%</span> EXPERT</span>
          <span><span style={{ color: '#e94560' }}>90-100%</span> MASTER</span>
        </div>
      </motion.div>
    </div>
  );
